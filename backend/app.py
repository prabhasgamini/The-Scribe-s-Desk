...
# List of reliable free models to try in order when rate-limited
FREE_MODELS = [
...
def chat():
    ...
    # Try each free model sequentially until one succeeds
    for model_name in FREE_MODELS:
        try:
            ...
            if response_content.strip():
                ...
                break  # Successful completion, exit model loop

        except (APIError, AuthenticationError) as e:
            last_error = str(e)
            print(f"Model '{model_name}' failed: {e}. Attempting next model...")
            continue
        except Exception as e:
            last_error = str(e)
            print(f"Unexpected error with '{model_name}': {e}. Attempting next model...")
            continue

    if not bot_response:
        return jsonify({"error": f"All free models are currently unavailable/rate-limited. Last error: {last_error}"}), 500

    # Strip reasoning thoughts (<think>...</think>) generated by reasoning models
